...
#4 входа состояние системы(тележки с маятником)
model = Chain(
    Dense(4, 32, relu),
    Dense(32, 64, relu),
    Dense(64, 64, relu),
...
# Размер обучающего батча, т.е. количество случайных примеров, 
# которые будут использованы в одной итерации оптимизации
batch_size = 100
# Количество полных циклов обучения (каждое обновление модели 
# после прохождения всех батчей).
epochs = 20000

# Цель - вернуть в вертикальное положение (целевой угол 0.0).
target_angle = 0.0
...
for epoch in 1:epochs
    ...
    # Градиенты потерь. Функция gradient() вычисляет градиенты функции потерь 
    # по отношению к параметрам модели. Градиенты необходимы для обновления весов модели.
    gs = gradient(() -> loss(random_batch, target_angle), Flux.params(model))
    # Обновляет параметры модели с использованием вычисленных градиентов. 
    Flux.Optimise.update!(opt, Flux.params(model), gs)
    # Каждые 10,000 эпох выводится текущая ошибка (потери) для мониторинга обучения.
    if epoch % 10000 == 0
        println("epoch_loss: ", loss(random_batch, target_angle))
    end
end
